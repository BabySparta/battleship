...
function addClassHover(cell) {
  if (ship === 6) return;
  if (cell.firstChild) {
    cell.classList.add("alreadyPlacedError");
    return;
  }
  const thisX = parseInt(cell.getAttribute("data-x"));
  const thisY = parseInt(cell.getAttribute("data-y"));
  let oneOver = document.querySelector(
    `.modalCell[data-x="${thisX + 1}"][data-y="${thisY}"]`
  );
  let twoOver = document.querySelector(
    `.modalCell[data-x="${thisX + 2}"][data-y="${thisY}"]`
  );
  let threeOver = document.querySelector(
    `.modalCell[data-x="${thisX + 3}"][data-y="${thisY}"]`
  );
  let fourOver = document.querySelector(
    `.modalCell[data-x="${thisX + 4}"][data-y="${thisY}"]`
  );
  if (direction === "vert") {
    oneOver = document.querySelector(
      `.modalCell[data-x="${thisX}"][data-y="${thisY + 1}"]`
    );
    twoOver = document.querySelector(
      `.modalCell[data-x="${thisX}"][data-y="${thisY + 2}"]`
    );
    threeOver = document.querySelector(
      `.modalCell[data-x="${thisX}"][data-y="${thisY + 3}"]`
    );
    fourOver = document.querySelector(
      `.modalCell[data-x="${thisX}"][data-y="${thisY + 4}"]`
    );
  }
...
function removeClassHover(cell) {
  ...
  if (!allValid) {
    cell.classList.remove("placeError");
    return;
  }
