function AImove(gameboard) {
  ...
  if (
    firstHit[1] + 1 < 10 &&
    !gameboard.board[firstHit[1] + 1][firstHit[0]].isHit
  ) {
    gameboard.recieveAttack(firstHit[0], firstHit[1] + 1);
    return [firstHit[0], firstHit[1] + 1];
  } else if (
    firstHit[0] + 1 < 10 &&
    !gameboard.board[firstHit[1]][firstHit[0] + 1].isHit
  ) {
    gameboard.recieveAttack(firstHit[0] + 1, firstHit[1]);
    return [firstHit[0] + 1, firstHit[1]];
  } else if (
    firstHit[1] - 1 >= 0 &&
    !gameboard.board[firstHit[1] - 1][firstHit[0]].isHit
  ) {
...
const hasOpenSpace = (shot, gameboard) => {
  if (
    (shot[1] + 1 < 10 && !gameboard.board[shot[1] + 1][shot[0]].isHit) ||
...
const findShips = (shot, gameboard, validSpots, allHits) => {
  // Vertical Ship
  if (
    isCoordInLocation(validSpots, [shot[0], shot[1] - 1]) ||
    isCoordInLocation(validSpots, [shot[0], shot[1] + 1])
  ) {
    shot = validSpots[0];
    while (
      ...
      shot[1] + 1 < 10 &&
      gameboard.board[shot[1] + 1][shot[0]].isHit &&
      validSpots.indexOf(shot) < validSpots.length - 1
    ) {
    ...
    if (shot[1] + 1 < 10 && gameboard.board[shot[1] + 1][shot[0]].isHit) {
      if (shot[1] - 1 >= 0 && !gameboard.board[shot[1] - 1][shot[0]].isHit) {
        gameboard.recieveAttack(shot[0], shot[1] - 1);
  ...
  // Horizontal Ship
    ...
    isCoordInLocation(allHits, [shot[0] - 1, shot[1]]) ||
    isCoordInLocation(allHits, [shot[0] + 1, shot[1]])
  ) {
    shot = validSpots[0];
    while (
      shot[0] - 1 >= 0 &&
      gameboard.board[shot[1]][shot[0] - 1].isHit &&
      shot[0] + 1 < 10 &&
      gameboard.board[shot[1]][shot[0] + 1].isHit &&
      validSpots.indexOf(shot) < validSpots.length - 1
    ) {
    ...
    if (shot[0] + 1 < 10 && gameboard.board[shot[1]][shot[0] + 1].isHit) {
      if (shot[0] - 1 >= 0 && !gameboard.board[shot[1]][shot[0] - 1].isHit) {
        gameboard.recieveAttack(shot[0] - 1, shot[1]);
        return [shot[0] - 1, shot[1]];
      }
    }
    if (shot[0] - 1 >= 0 && gameboard.board[shot[1]][shot[0] - 1].isHit) {
      if (shot[0] + 1 < 10 && !gameboard.board[shot[1]][shot[0] + 1].isHit) {
