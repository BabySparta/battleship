...
function playGame() {
  ...
  shipLocations.forEach((ship) => {
    ...
    placeOnDOM(
      user.gameboard.ships.slice(-1)[0],
      shipLocations.indexOf(ship),
      "userCell"
    );
  });
  ...
  oppBoard.forEach((cell) => {
    cell.addEventListener("click", () => {
      if (
        cell.lastChild &&
        (cell.lastChild.classList.contains("hit") ||
          cell.lastChild.classList.contains("miss"))
      )
        return;
      ...
      if (missLength < AI.gameboard.missed.length) placeMiss(cell);
        ...
        if (AI.gameboard.allShipsSunk()) {
          ...
          document.querySelector(".whoWon").textContent =
            "CONGRATULATIONS. YOU SUNK ALL ENEMY SHIPS AND WON THE BATTLE!";
          return;
        }
      }
      ...
      const AIcell = document.querySelector(
        `.userCell[data-x="${AIattack[0]}"][data-y="${AIattack[1]}"]`
      );
      if (AImiss < user.gameboard.missed.length) placeMiss(AIcell);
        ...
        if (user.gameboard.allShipsSunk()) {
          document.querySelector(".endModal").style.display = "flex";
          document.querySelector(".whoWon").textContent =
            "ARGHH! THE ENEMY HAS SUNK ALL YOUR SHIPS AND YOU HAVE TO RETREAT. REGROUP AND TRY AGAIN.";
        }
      }
...
const placeOnDOM = (ship, index, cellType) => {
  ...
  const origin = document.querySelector(
    `.${cellType}[data-x="${startX}"][data-y="${startY}"]`
  );
...
const displaySunkShip = (ship) => {
  ...
  const cellWithShip = document.querySelector(
    `.oppCell[data-x="${sunkX}"][data-y="${sunkY}"]`
  );
